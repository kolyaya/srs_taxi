\section{Водительское приложение}
  ...
  \subsection{Таксометр} \label{driver_app_taximeter_tab}
    ...
    \subsubsection{Вкладка при первом открытии} \label{driver_app_taximeter_tab_first_opening}
      \begin{itemize}

        \item{
          TITLE: Элементы вкладки при первом открытии\\
          \sr{Элементы этого окна описаны в таблице \ref{driver_app_taximeter_tab_first_opening_elements}.}\\
          PRIOR: MODERATE\\}
      ...
      \setlength{\extrarowheight}{2mm}
        ...
        \caption {Элементы вкладки при первом открытии} \label{driver_app_taximeter_tab_first_opening_elements} \\
          ...
          \hline \eltax{driver_element_driver_curr_coords}{} & Текущее местоположение водителя & \sr{Отображается в виде: \begin{itemize} \item Водитель авторизован: \begin{itemize} \item GPS включен: Индикатор зеленого цвета + GPS координаты текущего местоположения водителя. \item GPS выключен: Индикатор красного цвета + [Нет координат]. \end{itemize} \item Водитель не авторизован: Индикатор красного цвета + [Нет координат] \end{itemize}}\\ [2mm]    
          ...
          \hline \eltax{driver_element_start_button}{} & Кнопка “Старт” & \sr{По нажатию кнопки “Старт” интерфейс вкладки становится таким, как описано в \ref{driver_app_taximeter_tab_after_start_button}.}\\ [2mm]

          \hline

        \end{longtable}

    \subsubsection{Вкладка после нажатия кнопки “Старт”}  \label{driver_app_taximeter_tab_after_start_button}
      \begin{itemize}

        \item{
          TITLE: Элементы вкладки после нажатия кнопки “Старт”\\
          \sr{Новые элементы этого окна описаны в таблице \ref{driver_app_taximeter_tab_after_start_button_elements}.}\\
          PRIOR: MODERATE\\}
      ...
      \setlength{\extrarowheight}{2mm}
        ...
        \caption {Новые элементы вкладки после нажатия кнопки “Старт”} \label{driver_app_taximeter_tab_after_start_button_elements} \\
          ...
          \begin{itemize} 
            \item Интерфейс вкладки становится таким, как описано в \ref{driver_app_taximeter_tab_after_end_button}. 
            \item Прекращается изменение счетчика ELTAX-\ref{driver_element_this_order_time_after_start_button}, его значение сохраняется и отображается. 
            \item Прекращается изменение счетчика ELTAX-\ref{driver_element_this_order_dist_after_start_button}, его значение сохраняется и отображается. 
            \item Прекращается изменение счетчика ELTAX-\ref{driver_element_order_costs}, его значение сохраняется и отображается. 
            \item Все вкладки приложения, кроме “Таксометр”, становятся неактивными. 
          \end{itemize}}\\ [2mm]

          \hline \eltax{driver_element_order_info}{} & Информация о заказе & \sr{Выдвигаемая панель. В этой панели находятся следующие элементы: \begin{itemize} \item Заголовок - “ИНФОРМАЦИЯ О ТЕКУЩЕМ ЗАКАЗЕ” \item “Время” - отображается время начала выполнения заказа. \item “Откуда” - адрес, откуда началось выполнение заказа. \item “Куда” - адрес назначения заказа. \item “Примечания” - какие-либо примечания к текущему заказу. \end{itemize}}\\ [2mm]
          
          \hline

        \end{longtable}      
          
    \subsubsection{Вкладка после нажатия кнопки “Завершение”} \label{driver_app_taximeter_tab_after_end_button} 
      \begin{itemize}

        \item{
          TITLE: Элементы вкладки после нажатия кнопки “Завершение”\\
          \sr{Новые элементы этого окна описаны в таблице \ref{driver_app_taximeter_tab_after_end_button_elements}.}\\
          PRIOR: MODERATE\\}
      ...
      \setlength{\extrarowheight}{2mm}
        ...
        \caption {Новые элементы вкладки после нажатия кнопки “Завершение”} \label{driver_app_taximeter_tab_after_end_button_elements} \\
          ...
          \hline \eltax{driver_element_this_order_time_after_end_button}{} & Время, затраченное водителем на текущий заказ & \sr{Отображается сохраненное значение счетчика ELTAX-\ref{driver_element_this_order_time_after_start_button} после нажатия кнопки “Завершение”(ELTAX-\ref{driver_element_end_button}).}\\ [2mm]

          \hline \eltax{driver_element_this_order_dist_after_end_button}{} & Километраж текущего заказа & \sr{Отображается сохраненное значение счетчика ELTAX-\ref{driver_element_this_order_dist_after_start_button} после нажатия кнопки “Завершение”(ELTAX-\ref{driver_element_end_button}).}\\ [2mm]

          \hline \eltax{driver_element_order_costs_after_end_button}{} & Стоимость поездки & \sr{Отображается сохраненное значение счетчика ELTAX-\ref{driver_element_order_costs} после нажатия кнопки “Завершение”(ELTAX-\ref{driver_element_end_button}).}\\ [2mm] 

          \hline \eltax{driver_element_checkout_button}{} & Кнопка “Оплата” & \sr{По нажатию кнопки “Оплата”: \begin{itemize} \item Интерфейс изменяется на \ref{driver_app_taximeter_tab_first_opening}. \item Мобильное приложение делает запрос на сервер о завершении заказа, в котором передает итоговую стоимость поездки, коей является сохраненное значение счетчика ELTAX-\ref{driver_element_order_costs} после нажатия кнопки “Завершение”(ELTAX-\ref{driver_element_end_button}) или после изменения значения этого счетчика с помощью кнопки “Изменить”(ELTAX-\ref{driver_element_change_button}). В ответе сервер присваивает водителю статус “Свободен”. \item Все вкладки приложения вновь становятся активными. \end{itemize}}\\ [2mm]
